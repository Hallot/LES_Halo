...
//		__global float* p,
...
#ifdef MIC_SOR_KERNEL
	// This is a SOR kernel with twinned double buffering and fixed NTH, for the MIC
#ifndef COMBINED_KERNEL
	  __local float sor_chunks[NTH];
	    ...
	    float local_sor = 0.0F;
	        ...
	        unsigned int chunk_sz = ijkm/nunits; // this is about 8000
	        ...
	        for (unsigned int tc_idx = 0 ; tc_idx< n_tchunks; tc_idx++) {
	            ...
	            int4 ijk = calc_loop_iters(a_idx,im+2, jm+2, km+2,0,0,0) ;
	...
	if (i == im + 1) {
	    ...
	    if (j_lhs == jm + 1) {
	    	...
	    	sor_chunks[th_id]=local_sor;
	        barrier(CLK_LOCAL_MEM_FENCE);
			for (unsigned int ii=0;ii<NTH;ii++) {
				local_sor_acc+=sor_chunks[ii];
			}
...
float calc_reltmp_mp_rb(
	...
	if (nrd == 0) {
		...
		reltmp = calc_reltmp0(p, rhs, rhsav, cn1, cn2l, cn2s, cn3l,
...
#endif
#ifdef POISON_CACHE
	float tmp_res = poison_cache(uvw,i,j,k,ip,jp,kp);
	reltmp*=tmp_res;
#endif
	return reltmp;
}
...
#ifdef POISON_CACHE
float poison_cache(__global float* uvw,unsigned int i,unsigned int j,unsigned int k,const unsigned int ip,
		const unsigned int jp, const unsigned int kp) {
	float tmp_res=1.0;
	tmp_res += (uvw[FTNREF3D0(im-i, jm-j, km-k, ip + 3, jp + 3].s0+uvw[FTNREF3D0(im-i, j, k, ip + 3, jp + 3].s1+uvw[FTNREF3D0(i, jm-1, k, ip + 3, jp + 3].s2)*1e-16;
	return tmp_res;
}
