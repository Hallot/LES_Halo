        ...
        for (unsigned int v = 0; v < v_dim; v++) {
            ...
            for (unsigned int w = 0; w < h_w; w++) {
                ...
                if (i < jm-h_w && k < km && i > h_w-1) {
                    // west halo
                    array[i*im + w + k*im*jm + v*arr_sz] = buffer[v*v_sz + km*im*h_w*2 + i-h_w+w + k*(jm-2*h_w)*h_w];
                    // east halo
                    array[i*im + w + k*im*jm + im-h_w + v*arr_sz] = buffer[v*v_sz + km*im*h_w*2 + km*(jm-2*h_w)*h_w + i-h_w+w + k*(jm-2*h_w)*h_w];
                }
            }
        }
    }
    ...
    for (unsigned int gl_id = 0; gl_id < km * MAX(im,jm); gl_id++) {
        ...
        for (unsigned int v = 0; v < v_dim; v++) {
            for (unsigned int w = 0; w < h_w; w++) {
                if (i < im && k < km){
                    // north halo
                    buffer[v*v_sz + i + im*w + k*im*h_w] = array[i + im*w + k*im*jm + v*arr_sz];
                    // south halo
                    buffer[v*v_sz + km*im*h_w + i + im*w + k*im*h_w] = array[i + im*w + k*im*jm + im*(jm-h_w) + v*arr_sz];
                }

                if (i < jm-h_w && k < km && i > h_w-1) {
                    // west halo
                    buffer[v*v_sz + km*im*h_w*2 + (i-h_w)*h_w+w + k*(jm-2*h_w)*h_w] = array[i*im + w + k*im*jm + v*arr_sz];
                    // east halo
                    buffer[v*v_sz + km*im*h_w*2 + km*(jm-2*h_w)*h_w + (i-h_w)*h_w+w + k*(jm-2*h_w)*h_w] = array[i*im + w + k*im*jm + im-h_w + v*arr_sz];
                }
            }
        }
    }
...
int main(void) {
    ...
    const unsigned int v_sz = 2*km*im*h_w + 2*km*(jm-2*h_w)*h_w;
    const unsigned int ns_sz = im*h_w * km;
