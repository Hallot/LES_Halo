...
### OBSOLETE!
### This script has been replaced by genOclWrapperTemplate.pl

=info
This script takes a main program with annotations
...
and generates a new main with a call to a wrapper subroutine,
as well as the template for the wrapper subroutine, 
and the stub for the kernel.

Status:

* Need to extend this to work with code between subroutine calls.
* The current approach generates a single kernel stub for all subroutines together. It would be better initially to have a kernel stub per subroutine!
* Need a better name 
...
# Now I overload ArgMode to indicate if a value is const. Simply, any scalar is a const. 
        ...
        for my $arg_name (@var_names) {                
            ...
            if ($all or exists $kernel_args->{$arg_name}) {
                ...
                $ocl_args{$arg_name}{Dim} = $pt->{Dim};#$ndims;
...
#        say $wrapper_arg_name ,'=>',Dumper $wrapper_arg_decl;
